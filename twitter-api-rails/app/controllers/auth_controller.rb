class AuthController < ApplicationController
  ...
  def signup
    ...
    if !@user.save
      ...
      render json: UserBlueprint.render(@user, root: :user, meta: { token: access_token }), status: :created
    end
  end
  ...
  def login
    ...
    render json: UserBlueprint.render(@user, root: :user, meta: { token: access_token })
  end
  ...
  def refresh_session
    ...
    set_refresh_token_cookie(refresh_token)
    render json: UserBlueprint.render(@user, root: :user, view: :full, meta: { token: access_token })
  end
